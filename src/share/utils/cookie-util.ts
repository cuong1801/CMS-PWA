...
import { convertTimeToDate, getCurrentDay } from './date-util'
...
function addOrRemoveCookie(options: Unknown = {}, isAdd?: boolean, expiresIn?: number): void {
    Object.keys(options).forEach(function (key) {
        if (isAdd && options[key]) {
            ...
            if ((USERNAME === key && options[REFRESH_TOKEN]) || REFRESH_TOKEN === key)
                Cookies.set(key, `${options[key]}`, { expires: currentDay.add(1, 'y').toDate() })
            else Cookies.set(key, `${options[key]}`, { expires: convertTimeToDate(expiresIn) })
        } else {
            Cookies.remove(key)
        }
    })
